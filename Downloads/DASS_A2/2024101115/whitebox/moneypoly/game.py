...
class Game:
    ...
    def _apply_card(self, player, card):
        ...
        handlers = {
            "collect": self._handle_card_collect,
            "pay": self._handle_card_pay,
            "jail": self._handle_card_jail,
            "jail_free": self._handle_card_jail_free,
            "move_to": self._handle_card_move_to,
            "birthday": self._handle_card_birthday,
            "collect_from_all": self._handle_card_collect_from_all,
        }
    ...
    def _handle_card_collect(self, player, value):
        """Handle collecting money from the bank."""
        amount = self.bank.pay_out(value)
        player.add_money(amount)

    def _handle_card_pay(self, player, value):
        """Handle paying money to the bank."""
        player.deduct_money(value)
        self.bank.collect(value)

    def _handle_card_jail(self, player, value):  # pylint: disable=unused-argument
        """Handle going to jail."""
        player.go_to_jail()
        print(f"  {player.name} has been sent to Jail!")

    def _handle_card_jail_free(self, player, value):  # pylint: disable=unused-argument
        """Handle getting a Get Out of Jail Free card."""
        player.get_out_of_jail_cards += 1
        print(f"  {player.name} now holds a Get Out of Jail Free card.")

    def _handle_card_move_to(self, player, value):
        """Handle moving to a specific position."""
    ...
    def _handle_card_birthday(self, player, value):
        """Handle birthday card - collect from each other player."""
        for other in self.players:
            if other != player and other.balance >= value:
                other.deduct_money(value)
                player.add_money(value)

    def _handle_card_collect_from_all(self, player, value):
        """Handle collecting from all other players."""
        for other in self.players:
            if other != player and other.balance >= value:
                other.deduct_money(value)
                player.add_money(value)
