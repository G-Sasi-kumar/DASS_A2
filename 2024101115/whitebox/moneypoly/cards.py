"""Card decks used by MoneyPoly chance and community-chest tiles."""
...
class CardDeck:
    ...
    def draw(self):
        """Draw the next card from the deck, cycling when the deck is exhausted."""
        if not self.cards:
            return None
    ...
    def peek(self):
        """Return the next card without advancing the deck index."""
        if not self.cards:
            return None
